...
**HabitMiner** addresses this gap by introducing a lightweight, privacy-preserving, context-aware framework. By combining **Stay Point Extraction**, **Spatio-Temporal Clustering (ST-DBSCAN)**, **Markov-based Sequence Mining**, and **Information Theory Metrics (Shannon Entropy & KL-Divergence)**, HabitMiner learns daily patterns, measures routine stability, and detects behavioral drift—all executed via edge-based computing on the user's mobile device.
...
flowchart TD
    subgraph SENSING["1. Mobile Sensor Stream"]
        A[Raw GPS Trajectory Stream<br/>Lat, Lon, Timestamp, Altitude]
    end

    subgraph PREPROCESSING["2. Stay Point Extraction"]
        B[Filtering & Windowing]
        C[Stay Point Detection<br/>D_th, T_th Parameters]
    end

    subgraph CLUSTERING["3. Spatio-Temporal Discovery"]
        D[ST-DBSCAN Clustering<br/>Spatial Eps1, Temporal Eps2, MinPts]
        E[Semantic POIs Discovery<br/>Home, Work, Gym, Cafe, etc.]
    end

    subgraph MINING["4. Routine Mining & Modeling"]
        F[Spatial Sequence Formatter]
        G[Markov Chain Transition Model<br/>First-Order & Higher-Order States]
    end

    subgraph ANALYSIS["5. Routine Stability & Drift"]
        H[Shannon Entropy H_X<br/>Predictability & Routine Stability]
        I[KL Divergence D_KL<br/>Routine Drift & Behavioral Shift Detection]
    end
...
* 📍 **Extract Meaningful Stay Points**: Filter high-frequency raw GPS trajectories into discrete stationary episodes defined by spatial radius $D_{th}$ and duration $T_{th}$.
* 🏙️ **Semantic Place Discovery**: Cluster stay points using **ST-DBSCAN** to automatically uncover meaningful Points of Interest (POIs) such as home, workspace, or recreational spots.
* 🔄 **Sequential Pattern Mining**: Mine recurring daily/weekly movement sequences and model location transition probabilities using Markov Chains.
* 📊 **Routine Stability & Drift Measurement**: Apply **Shannon Entropy** to quantify daily routine predictability and **Kullback-Leibler (KL) Divergence** to detect behavioral modifications over time.
* 📱 **Privacy-Preserving Edge Architecture**: Maintain complete user data sovereignty by running on-device processing via an Android prototype powered by Kotlin, Room DB, and WorkManager.
...
│   ├── GeoLife GPS Trajectory Parser
│   ├── Stay Point Extractor & ST-DBSCAN Implementation
│   ├── Markov Chain Routine Predictor
│   └── Entropy & KL-Divergence Analytical Engine
│
├── 🔹 Phase 2: On-Device Android Prototype (Kotlin)
│   ├── Passive Location Collector & Adaptive Sensing
│   ├── Room Database Schema for Stay Points & Trajectories
│   ├── Trajectory Replay Mode for Simulation
...
└── 🔹 Phase 3: Evaluation & Visualization Dashboard
    ...
    └── Prediction Accuracy & Performance Benchmarking
```
...
| **Mobile Prototype (Android)** | Kotlin, Android SDK, Room Persistence Library, WorkManager, Google Play Location Services |
| **Algorithms** | Stay Point Extraction, ST-DBSCAN, Markov Chains, Shannon Entropy, KL-Divergence |
...
│   ├── METHODOLOGY.md     # Mathematical formulation of algorithms
...
- 📐 [**Methodology Specification**](docs/METHODOLOGY.md) - Mathematical definition of Stay Points, ST-DBSCAN, Markov Chains, Entropy, and KL Divergence.
- 🏗️ [**Architecture Document**](docs/ARCHITECTURE.md) - Deep dive into system layers, privacy guardrails, and Android architecture.
...
- ❌ **No High-Frequency Tracking**: Adaptive sampling strategies are employed to minimize GPS power consumption.
